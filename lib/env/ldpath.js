...
var init = function () {
  if (xPlatform.getOs () !== 'win') {
    typeList.push ('posix');
    envLdPath.posix = process.env.LD_LIBRARY_PATH || null;
    ldPathList.xcraft.posix = envLdPath.posix ? envLdPath.posix.split (path.delimiter) : [];
    ldPathList.devroot.posix = [];
  }
  if (xPlatform.getOs () === 'darwin') {
    typeList.push ('darwin');
    envLdPath.darwin = process.env.DYLD_LIBRARY_PATH || null;
    ldPathList.xcraft.darwin = envLdPath.darwin ? envLdPath.darwin.split (path.delimiter) : [];
